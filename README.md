...
`vcable-coreaudio`の`PcmPlaybackStream`／`PcmCaptureStream`は、リンクした同一プロセスからCore Audioを直接利用するAPIです。PCM用IPCではなく、音声転送に`vcabled`やUnixソケット、共有メモリを経由しません。

PCMはinterleaved `f32`です。`PcmStreamConfig`には対象デバイスのUID、サンプルレート、チャンネル数を明示し、Core Audio上の形式と完全に一致させる必要があります。デバイスの暗黙選択、別デバイスへの切り替え、サンプルレート変換、チャンネル変換は行いません。

- `PcmPlaybackStream`はプロセスからCore Audioデバイスのoutput側へ書き込みます。VCableでは、このPCMが同じ仮想デバイスのinput側へループバックされます。
- `PcmCaptureStream`はCore Audioデバイスのinput側からプロセスへ読み出します。VCableでは、同じ仮想デバイスのoutput側へ書かれたループバックPCMを取得します。
- Streamハンドルは`Send`で、別のTokioワーカーを含む別スレッドへ移動できます。Streamも、対応する`PcmWriter`／`PcmReader`も`Clone`しない単一所有モデルです。

`target_fill_frames`はドリフト補正の中心値であると同時に、開始時とunderrun復帰時のpriming閾値です。線形補間には2フレーム必要なため、実際には`max(target_fill_frames, 2)`フレームまで待ちます。この値はリング内のフレームだけでなく、リサンプラーが`current`／`next`として保持するフレームも含む総バッファ量です。`ring_fill_frames`メトリクスだけはリング内のフレーム数を表し、保持中の`current`／`next`を含みません。

playbackはpriming中に無音を出力し、captureの`PcmReader::try_read_interleaved`はPCMを消費せず`0`を返します。一度primedになった後にPCMが枯渇するとunprimedへ戻り、同じ閾値まで再primingします。初回primingの無音は`underrun_frames`に加算せず、一度primedになった後に枯渇で出力できなかったplaybackフレームだけを加算します。暗黙の別処理への切り替えはありません。
